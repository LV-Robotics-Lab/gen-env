...
def test_package_verifier_rejects_manifest_paths_outside_package_root(tmp_path: Path) -> None:
    _, spec, resolved = solved_case()
    ...
    symlink = package_root / "outside-link"
    symlink.symlink_to(secret)

    manifest_path = package_root / "package_manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    package_records = manifest["files"]
    attacks = (
        "../outside-secret.txt",
        str(secret.resolve()),
        symlink.name,
    ...
    for declared in attacks:
        manifest["files"] = [
            *package_records,
            {"path": declared, "sha256": secret_digest, "bytes": secret.stat().st_size},
        ]
        manifest_path.write_text(json.dumps(manifest), encoding="utf-8")

        report = verify_package(package_root)
        check = next(item for item in report["checks"] if item["path"] == declared)

        assert report["status"] == "fail"
        assert check["path_within_package"] is False
