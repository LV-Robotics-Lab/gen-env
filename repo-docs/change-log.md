...
- 2026-09-08：将已有场景图的位置求解、干预式稳定化和双时间步自由验收接入现有物理入口及媒体阶段，扩展多支撑/堆叠/方位门控、阶段恢复与终态渲染门控；三个新任务完成真实回放和展示。

- 2026-09-07：将位置求解用于鼠标重建资产，新增有界支撑 SDF 显式参数及实际加载记录；在新任务完成真实基线/半时间步物理通过、全程足迹覆盖和独立终态展示，保留旧失败及准备错误，记录桌面视觉差异。

- 2026-09-07：新增保留原朝向的多场景位置求解器，提供显式局部支撑面、完整足迹约束、多固定根与堆叠、有界回溯、独立 CLI 和反例测试；区分几何提议、真实盒体回放与媒体生产验收。

- 2026-09-07：记录鼠标真实接触振动对照，定位桌面碰撞表示与轴重排后未反转面绕序的缺陷；区分诊断代理稳定和正式验收通过，保留 physics_failed，未改生产几何或门槛。
...
- 2026-09-08：按用户要求归档并清空 output，开展文本／图片／视频复测；明确原生文本物理入口与标准 URDF 工作流的接入边界，保存并验证单图奇数尺寸编码补丁。证据目录：`data/media_acceptance/full_pipeline_retest_20260908_011810/`。

- 2026-09-08：修复 Genesis 资产选择对 `openai/gpt-4o` 的本地误拒绝，保留完整模型名用于请求与证据；其他模型及非 Chat 模式仍拒绝。资产选择专项 57 passed，根测试套件 385 passed，Ruff 与 diff 检查通过；本次未重新调用在线模型或运行物理验收。

- 2026-09-08：按用户要求将媒体重建默认支撑恢复为上游平面语义；新增已有重建直接转换入口，保留原始 JSON、位姿和平面，不检索 support_0。原有限支撑流程改为显式 retrieved 模式。真实鼠标 Genesis 预览 120/120 不同帧，03/04 未运行；专项 43 passed、2 skipped，根套件 385 passed，Ruff 与 diff 检查通过。

- 2026-09-09：物理验收判据分类与有界修复搜索。新增 `physics_criteria.py`（判据分为 scene_truth / numerics_coupled / diagnostic，依据是实测灵敏度：同一静置场景把 `constraint_timeconst` 从 0.008 扫到 0.05，稳态穿透变化 20 倍而 `drift_rate` 只变 1.6 倍）与 `physics_repair_agent.py`（仅当失败全部属于 numerics_coupled 时才调 gpt-4o 提数值配置，回复经白名单、注册表标定范围、`2*dt` 稳定性下限、`satisfiable()` 与去重校验后才执行；阈值不在其 schema 内）。`repair_numerics` 补 4ms 档位与 `synthesize`，agent 自造组合标记 `agent_proposed` 并被 `verify_evidence` 拒绝作为资产资质。单资产入口加 `--at-rest`（贴面释放，验收不变）。Genesis 内置平面成为合法固定根：导入器从实测几何推导贴地关系，`validate_imported_scene` 补建地面实体，`upstream` 模式据此接通物理与终态渲染，并新增 `visual_support.py` 提供 `collision=False` 的渲染专用支撑面（顶面高度由 `verify()` 从写出的网格复核）。真实验证：鼠标场景保留 SimFoundry 原始平面通过场景级物理（十项判据全过，支撑力 0.58859 N 对重量 0.58860 N），四阶段全部 passed；agent 循环在单资产入口实测一轮收敛。已知未解决：桌子+杯子需 tau≈0.05 而本场景需 ≤0.03，单一全局常数在固定 1 mm 穿透红线下可能无法同时满足；落体测试的穿透判据在 dt=0.004 下不可满足（`test_authored_contact_margin_control` 因此失败，改动前即已失败，经 stash 对照确认）。
